...
track_state_container_types::buffer fitting_algorithm<fitter_t>::operator()(
    ...
    if (n_tracks > 0) {
        ...
        using lv_buf_t = vecmem::data::vector_buffer<float4>;
        using off_buf_t = vecmem::data::vector_buffer<unsigned int>;
        lv_buf_t cand_lv_buffer(static_cast<lv_buf_t::size_type>(offsets.back()),
                                m_mr.main);
        off_buf_t offset_buffer(
            static_cast<off_buf_t::size_type>(offsets.size()), m_mr.main);
        ...
        vecmem::data::vector_view<const unsigned int> offsets_view(
            static_cast<decltype(offsets_view)::size_type>(offsets.size()),
            offsets.data());
